...
export function QuantumLab({
  ...
  return (
    <aside className="w-full rounded-xl border border-line bg-surface p-5">
      ...
      <div className="grid gap-x-6 gap-y-4 lg:grid-cols-3">
        ...
        {/* Column 2 — probability over iterations */}
        {result && snap ? (
          ...
          <p className="hidden text-xs leading-relaxed text-muted lg:block">
            「Grover を実行」を押すと、反復ごとの候補確率の推移がここに表示されます。
          </p>
        )}

        {/* Column 3 — amplitude distribution */}
        {result && snap && (
